...
식사하는 철학자들 문제는 고전적인 동기화 문제로 간주하는데, 그 이유는 실용적으로 중요하거나 혹은 컴퓨터 과학자들이 철학자를 싫어해서가 아니라 많은 부류의 병행 제어 문제의 한 예이기 때문이다. 그것은 교착 상태와 기아를 발생시키지 않고 여러 스레드에게 여러 자원을 할당해야 할 필요를 단순하게 표현한 것이다. 

![dining_philosophy](../assets/img/dining_philosophy.png)

식사하는 철학자들 문제는 다음과 같은 사진처럼 시작한다. 5명의 철학자들이 있고 각 철학자들 사이에는 한 개의 젓가락이 있다. 특정한 철학자가 밥을 먹기 위해서는 왼쪽의 젓가락, 오른쪽의 젓가락 두 개를 집어야 한다.

하지만, 만약 한 번에 5명의 철학자들이 모두 배가 고파 동시에 자신의 왼쪽에 있는 젓가락을 집었다고 가정한다. 그리고 밥을 먹기 위해서는 오른쪽의 젓가락도 필요하므로 오른쪽에 있는 젓가락을 들려고 했는데, 자신의 오른쪽에 있던 젓가락은 자신의 오른쪽에 앉아있는 철학자의 왼쪽 손에 들려있기 때문에 그 사람이 젓가락을 놓기를 기다린다. 이렇게 모든 철학자들이 자신의 오른편에 있는 사람의 작업이 끝나기를 기다리게 되기 때문에 교착상태(Deadlock)에 빠지게 된다. 모든 철학자들이 굶어죽게(Starvation) 되는 것이다.
...
이렇게 5개의 세마포어를 만들 수 있다. 배열의 인덱스는 각 젓가락들의 번호를 의미하고 각 젓가락은 한 번에 한 명만 집어들 수 있기 때문에 1로 초기화 된다. 
...
1. 왼쪽에 있는 젓가락을 들 수 있는 지 확인한다.
2. 들 수 있다면 오른쪽 젓가락을 들기 위해 계속 진행하고, 다른 철학자가 오른쪽 젓가락을 들고 있다면 내려놓을 때까지 대기한다.
3. 두 개의 젓가락을 모두 획득했다면 작업을 수행한다.
4. 작업을 마치고 두 젓가락을 모두 내려놓는다.

하지만, 이 해결안은 인접한 두 철학자가 동시에 식사하지 않는다는 것을 보장하지만, 교착상태를 야기할 가능성이 있다. 초입에 설명했던 경우를 해결하지 못하기 때문이다. 5명의 철학자들이 모두 동시에 배가 고파져서 각각 자신의 왼쪽 젓가락을 잡게 되면 `chopstick`의 모든 원소들은 0이 될 것이고 각 철학자가 자신의 오른쪽을 잡으려고 영원히 기다려야하는 교착상태에 빠지게 된다.

결국 세마포어만으로는 해당 문제를 해결할 수 없다. 때문에 추가적인 전략이 필요하다.

- 최대 4명의 철학자만이 테이블에 동시에 앉을 수 있도록 한다.
- 한 철학자가 젓가락을 두 개를 모두 집을 수 있을 때만 젓가락을 집도록 허용한다. (이를 위해서는 철학자는 임계구역 안에서만 젓가락을 집어야 한다.)
- 비대칭 해결안을 사용한다. 즉, 홀수 번호의 철학자는 먼저 왼쪽 젓가락을 집고 다음에 오른쪽 젓가락을 집는다. 반면에 짝수 번호의 철학자는 오른쪽 젓가락을 집고 다음에 왼쪽 젓가락을 집는다.
