...
### 해당 브랜치에서 작업이 길어질 것 같은 경우

만약 내가 로그인 작업을 맡았고 해당 작업에 3일 정도가 소요될 것 같다고 가정한다.

그렇다면 작업을 시작하는 지금 새로운 브랜치를 만들고, 해당 브랜치로 가서 작업을 시작해야 할 지 

아니면 일단은 develop 브랜치에서 작업을 하고 작업이 마무리 되면 그때 새로운 브랜치를 만들고, 새로운 브랜치로 옴겨가서 PR을 보내야 하는 지에 대해 궁금해졌다.

일단 정답은 전자이다.

작업을 시작하면서 브랜치를 만들고 해당 브랜치로 가서 작업을 해나간다. 

그렇다면, 여기에서 또 다른 의문이 들 수 있다. "내가 로그인 작업을 하게 되는 3일 동안 develop 브랜치에 머지된 파일들은 새로운 브랜치에 반영이 되지 못할 텐데 문제가 되지 않을까??"

당연히 문제가 된다. 때문에 add. commit. push를 하기 직전에 `git pull origin develop` 명령어를 통해 한번 동기화 시키고 푸시할 필요가 있다. 이렇게 해야지 충돌이 생겼을 때 미리 해결할 수 있고, 3일 간의 변경 점들을 모두 반영한 PR을 보낼 수 있게 된다.

---

### 불필요한 머지 커밋

![git_merge_commit](../assets/img/git_merge_commit.png)

위의 예제처럼 3일 작업하고 (실제로 3일이나 작업하지는 않았다.) pull을 한 번 당기고 푸시하면 이렇게 필요없는 머지에 대한 커밋 메시지 하나가 추가된다. 

하지만, 우리는 머지는 수행시키고 싶지만 저 필요없는 커밋 메시지는 만들고 싶지 않다.

그럴 때 `git pull --ff-only origin develop`을 사용하면 된다. 하지만, ff only 옵션은 특정한 경우에만 작동이 가능하다.

https://sanghye.tistory.com/43

내가 설명하기보다는 이 글을 한번 읽으면 큰 도움이 될 것 같다.

비슷한 기능을 하지만 히스토리를 수정하는 --rebase 옵션도 존재한다.
