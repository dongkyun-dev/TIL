...
## 4. 렌더링 트리에서 레이아웃을 실행하여 각 노드의 기하학적 형태를 계산한다.(Layout 혹은 flow 단계)

이 레이아웃 단계에서, 기기의 뷰포트 내에서 렌더링 트리의 노드들에 대한 위치와 크기가 계산된다. 
...
5. 추가 내용 (2021.06.11) (유튜브 우아한 Tech 채널의 [[10분 테코톡] ☕️ 체프의 브라우저 렌더링](https://www.youtube.com/watch?v=sJ14cWjrNis))

   > 기존 내가 정리할 내용에서는 reflow와 repaint 단계만을 소개했다. 하지만, 알고 보니 한 단계가 더 존재했는데 바로 **Composite**이다. 
   >
   > 
   >
   > ![rerender](../assets/img/rerender.jpg)
   >
   > - reflow(relayout)부터 다시 발생하는 경우
   >
   > 	>주로 요소의 크기나 위치가 바뀔 때, 혹은 브라우저 창의 크기가 바뀌었을 때 다시 발생한다.
   > 	
   > - repaint부터 다시 발생하는 경우
   >
   >   > 주로 배경 이미지나 텍스트 색상, 그림자 등 레이아웃의 수치를 변화시키지 않는 스타일의 변경이 일어났을 때 발생
   >
   > - 레이어의 합성만 다시 발생하는 경우 (Composite)
   >
   >   > 레이아웃과 페인트를 수행하지 않고 레이어의 합성만 발생하기 때문에 성능상으로 가장 큰 이점을 가진다. 
   >   >
   >   > 더 자세하게 말하자면, 일부 속성은 CPU에서 레이아웃 재계산 없이 GPU에서 처리 가능하다. 레이아웃 계산 없이 GPU에서 처리 되므로 빠르게 렌더링 된다. 특히 애니메이션에서 유리하다.
   >   >
   >   > transform, opacity 와 같은 요소들이 composite만을 수행하는 것들이다.
   >
   > </br>
   >
   > https://csstriggers.com/
   >
   > >이 사이트에서 어떤 CSS 속성이 어떤 종류에 해당하는지 브라우저별로 구분되어 들어있다. 실제 효율적인 CSS 활용이 필요하다면 해당 사이트에서 같은 기능을 하지만 더 효율적인 속성으로 선택해서 사용하면 되겠다.
