...
이를 이해하기 위해서는 `Well-Known Port Number`에 대해 알아야 한다.
...
실질적으로 우리가 53번 포트를 통해 접근하는 DNS는 Local DNS이다. 하지만, DNS는 Local만 있는 것이 아니며, Local DNS에 모든 정보들이 저장되어 있는 것이 아니다. 떄문에 또 다른 DNS로 해당 도메인 이름에 대한 IP 주소를 찾을 때까지 재귀적인 접근이 필요하다.

이에 대한 설명이 [아마존의 DNS에 대한 설명](https://aws.amazon.com/ko/route53/what-is-dns/) 하단에 잘 정리되어 있다. 하지만, 처음 이 아마존의 글을 읽으면 이해하기 상당히 어렵다. 이 때문에 다른 설명을 찾게 되었고 굉장히 좋은 설명을 찾게 되어서 아래에 그대로 옮긴다. (출처: 넷매니아즈, https://www.netmanias.com/ko/post/blog/5353/dns/dns-basic-operation)
